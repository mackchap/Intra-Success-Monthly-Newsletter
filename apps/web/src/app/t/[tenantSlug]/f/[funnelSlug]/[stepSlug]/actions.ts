...
export async function leadCaptureAction(formData: FormData) {
  ...
  if (!funnelId || !funnelStepId || !tenantSlug || !funnelSlug || !stepSlug) {
  ...
  if (nextStep) {
    redirect(`/t/${tenantSlug}/f/${funnelSlug}/${nextStep.slug}?lead=${contact.id}`);
  }
  redirect(`/t/${tenantSlug}/f/${funnelSlug}/${stepSlug}?lead=${contact.id}&submitted=1`);
}
