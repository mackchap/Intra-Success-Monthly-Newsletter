...
export default async function TasksPage({ params }: { params: Promise<{ tenantId: string }> }) {
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">
                ...
                {task.contact ? (
                  <>
                    {" · "}
                    <Link href={`/a/${tenantId}/staff/contacts/${task.contact.id}`} className="text-brand-600">
                ...
                {task.deal ? (
                  <>
                    {" · "}
                    <Link href={`/a/${tenantId}/staff/deals/${task.deal.id}`} className="text-brand-600">
                      {task.deal.title}
                    </Link>
                  </>
                ) : null}
