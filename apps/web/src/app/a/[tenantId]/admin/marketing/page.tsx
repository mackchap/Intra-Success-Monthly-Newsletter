...
export default async function AdminMarketingPage({
  ...
  const [accounts, campaigns] = await Promise.all([
    prisma.socialAccount.findMany({ where: { tenantId, disconnectedAt: null }, orderBy: { connectedAt: "desc" } }),
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <section>
        ...
        {accounts.length === 0 ? (
          <p className="text-sm text-slate-500">
            ...
            <a href={`/api/social/meta/connect?tenantId=${tenantId}`} className="text-brand-600">
          ...
          <>
            ...
            <a href={`/api/social/meta/connect?tenantId=${tenantId}`} className="mt-2 inline-block text-sm text-brand-600">
        ...
        <ul className="mt-6 flex flex-col gap-2">
          {campaigns.map((campaign) => (
            <li key={campaign.id} className="flex items-center justify-between rounded-md border border-slate-200 p-3 text-sm">
              <Link href={`/a/${tenantId}/admin/marketing/${campaign.id}`} className="font-medium text-brand-600">
                {campaign.name}
              </Link>
      ...
      <InsightsAdvice tenantId={tenantId} />
    </div>
  );
}
