...
export default async function AdminCampaignDetailPage({
  ...
  const [campaign, accounts] = await Promise.all([
    ...
    prisma.socialAccount.findMany({ where: { tenantId, disconnectedAt: null }, orderBy: { displayName: "asc" } }),
  ]);
    ...
    <div className="flex flex-col gap-8">
      <div>
        <p className="text-sm text-slate-500">
          <Link href={`/a/${tenantId}/admin/marketing`} className="text-brand-600">
            ← Marketing
          </Link>
        </p>
      ...
      <section>
        ...
        {accounts.length === 0 ? (
          ...
          <DraftRequestForm tenantId={tenantId} campaignId={campaign.id} accounts={accounts} />
        )}
      </section>
