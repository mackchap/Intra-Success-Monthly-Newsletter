...
export default async function AdminFunnelsPage({ params }: { params: Promise<{ tenantId: string }> }) {
        ...
        <tbody>
          {funnels.map((funnel) => (
            <tr key={funnel.id} className="border-t border-slate-100">
              <td className="py-2">
                <Link href={`/a/${tenantId}/admin/funnels/${funnel.id}`} className="font-medium text-brand-600">
                  {funnel.name}
                </Link>
              </td>
