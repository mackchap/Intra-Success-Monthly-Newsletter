...
export default async function AdminFunnelDetailPage({
  params,
}: {
  ...
  const [funnel, products] = await Promise.all([
    prisma.funnel.findFirst({ where: { id, tenantId }, include: { steps: { orderBy: { order: "asc" } } } }),
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">
            /t/{tenant.slug}/f/{funnel.slug}
          </p>
        </div>
        <div className="flex items-center gap-3">
          <Link href={`/a/${tenantId}/admin/funnels/${funnel.id}/analytics`} className="text-sm font-medium text-brand-600">
            Analytics
          </Link>
            ...
            return (
              <div key={step.id} className="rounded-lg border border-slate-200 p-4">
                <div className="flex items-center justify-between">
                  ...
                  <Link href={`/t/${tenant.slug}/f/${funnel.slug}/${step.slug}`} target="_blank" className="text-xs text-brand-600">
                    View live ↗
                  </Link>
                </div>
