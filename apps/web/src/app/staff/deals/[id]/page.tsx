  ...
  return (
    <div className="flex flex-col gap-8">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">
            <Link href={`/staff/contacts/${deal.contact.id}`} className="text-brand-600">
              {contactName(deal.contact)}
            </Link>
            {deal.company ? (
              <>
                {" · "}
                <Link href={`/staff/companies/${deal.company.id}`} className="text-brand-600">
                  {deal.company.name}
                </Link>
              </>
            ) : null}
      ...
      <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
        <TasksPanel tasks={deal.tasks} dealId={deal.id} />
        <NotesPanel notes={deal.notes} dealId={deal.id} />
      </div>
