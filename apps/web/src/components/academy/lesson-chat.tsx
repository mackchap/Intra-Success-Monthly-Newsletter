...
import { askStudentSupportAction } from "@/app/portal/courses/[id]/lessons/[lessonId]/chat-actions";
...
export function LessonChat({ lessonId }: { lessonId: string }) {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const answer = await askStudentSupportAction(lessonId, history, question);
  ...
  return (
    <div className="flex flex-col gap-3 rounded-lg border border-slate-200 p-4">
      <h2 className="font-medium">Ask about this course</h2>

      {messages.length === 0 && (
        <p className="text-sm text-slate-500">
          Ask a question about this course&apos;s content and an AI assistant will search the lessons for you.
        </p>
      )}
